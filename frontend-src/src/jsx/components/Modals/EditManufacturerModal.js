    ...
    useEffect(()=>{
        if(!editData?.name){
            ...
            API.getById(editData.id).then(res => {
                ...
                setName(temp.name);
                setCountry(Number(temp.country.id));
    ...
    return (
        <Modal className="modal fade"  show={editModal} onHide={setEditModal} >
            <div className="" role="document">
                <form onSubmit={e => edit(e, editData.id, {name, country_id: country, is_active: isActive, deleted: isDeleted})}>
...
export default connect(mapStateToProps)(EditManufacturerModal);