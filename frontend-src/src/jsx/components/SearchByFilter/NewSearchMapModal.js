...
export default function NewSearchMapModal(props) {
    const {
        API,
        date,
        show,
    ...
    useEffect(() => {
        let temp = true;
        if (show) {
            if (dataType === 1 || dataIDList[list[selectedCheckbox]].length > 10) {
                setIsCorrectDate({
                    monthly: false,
                    quarterly: false,
                    yearly: false
            ...
            if (temp) {
                getList(chartType, periodType, selectedRegion)
            }
        } else {
            setActive(0)
            setPeriodType("")
    ...
    const handleChangePeriodType = (periodType) => {
        ...
        setChartData({ datasets: [], labels: [] })
        getList(tempChartType, tempPeriodType, selectedRegion)
    }
    ...
    const handleSelectRegion = (region_id) => {
        getList(chartType, periodType, region_id)
    }



    const getList = (chartType = "pie", periodType, region_id) => {
        setLoading(true);
        const temp = {
            is_active: true,
            is_deleted: false,
        ...
        SearchService.graphReport(temp).then(res => {
            if (res.data.count) {
                ...
                setData([])
                convertDataToChartData([], active, chartType, periodType, chartDataType)
            }
            setLoading(false);
        }).catch(err => {
    ...
    return (
        ...
        >
            <div className="modal-header">
                <h4 className='m-0'>{TR(lang, "chart.chart_map_title")}</h4>
