...
use App\Models\Views\DistributerView;
...
class DistController extends Controller
{
    ...
    public function GetByID(Request $request)
    {
        try {
            ...
            $data = Distributor::findOrFail($dataID);
            if (empty($data)) {
                $message = "Что-то пошло не так! Пожалуйста, попробуйте позже!";
                $error = "Data not found";
                return _sendError(404, $message, $error);
    ...
    public function getSearchKeyword(Request $request)
    {
        try {
            ...
            if ($request->has('idList') && $inputs['idList']) {
                $data = DistributerView::scopeSearchByID(explode(',',$inputs['idList']), $isActive, $isDeleted);
            }
            else {
                $data = DistributerView::
                scopeSearchByWord(
                    Str::upper($keyword),
                    $dtIDList,
                    $scID,
    ...
    public function AddData(Request $request)
    {
        try {
            ...
            $data->distributor_inn = $inputs['distributor_inn'] ?? null;
    ...
    {
        try {
            Excel::import(new DistributorImport, $request->file);
    ...
    public function ChangeStatus(Request $request)
    {
        try {
            ...
            $data = Distributor::scopeDeleteIt($dataID, $inputs["is_active"] == true ? 1 : 0, $inputs["deleted"] == true ? 1 : 0);
            switch ($data) {
    ...
    public function RemoveIdList(Request $request)
    {
        try {
            ...
            $data = Distributor::whereIn('id', $dataID['dataID']);
            $data->delete();
            $message = "Data removed successfully";
            return _sendResponse(201, $message);
        } catch (\Throwable $error) {
    ...
    public function RemoveListStatus(Request $request)
    {
        try {
            ...
            $isActive = $inputs["is_active"] == true ? 1 : 0;
            $isDeleted = $inputs["deleted"] == true ? 1 : 0;
            $data = Distributor::scopeActiveDeactivateList($inputs['dataID'], $isActive, $isDeleted);
            if ($data) {
