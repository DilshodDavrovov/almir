...
class DTController extends Controller
{
    ...
    public function GetByID(Request $request)
    {
        try {
            ...
            $data = DrugType::findOrFail($dataID);
            if (empty($data)) {
                $message = "Что-то пошло не так! Пожалуйста, попробуйте позже!";
                $error = "Data not found";
                return _sendError(404, $message, $error);
    ...
    public function AddData(Request $request)
    {
        try {
            ...
            $rules = [
                'name' => 'required|string|max:255|unique:drug_types,name,' . $dataID,
            ];
    ...
    public function getSearchKeyword(Request $request)
    {
        try {
            ...
            if ($request->has('idList') && $inputs['idList']) {
                $data = DTView::scopeSearchByID(explode(',',$inputs['idList']), $isActive, $isDeleted);
            }
            else {
                $data = DTView::scopeSearchByWord(Str::upper($keyword), $isActive, $isDeleted);
            }
            $message = "Данные успешно сохранены";
            return _sendResponse(201, $message, $data);
        } catch (\Throwable $error) {
    ...
    {
        try {
            Excel::import(new DTImport, $request->file);
    ...
    public function ChangeStatus(Request $request)
    {
        try {
            ...
            $data = DrugType::scopeDeleteIt($dataID, $inputs["is_active"] == true ? 1 : 0, $inputs["deleted"] == true ? 1 : 0);
            switch ($data) {
    ...
    public function RemoveIdList(Request $request)
    {
        try {
            ...
            $data = DrugType::whereIn('id', $dataID['dataID']);
            $data->delete();
            $message = "Data removed successfully";
            return _sendResponse(201, $message);
        } catch (\Throwable $error) {
    ...
    public function RemoveListStatus(Request $request)
    {
        try {
            ...
            $isActive = $inputs["is_active"] == true ? 1 : 0;
            $isDeleted = $inputs["deleted"] == true ? 1 : 0;
            $data = DrugType::scopeActiveDeactivateList($inputs['dataID'], $isActive, $isDeleted);
            if ($data) {
