...
use App\Imports\DTGImport;
use App\Models\DrugTsGroup;
use App\Models\Views\DTGView;
...
class TPGController extends Controller
{
    ...
    public function GetByID(Request $request)
    {
        try {
            ...
            $data = DrugTsGroup::findOrFail($dataID);
            if (empty($data)) {
                $message = "Что-то пошло не так! Пожалуйста, попробуйте позже!";
                $error = "Data not found";
                return _sendError(404, $message, $error);
    ...
    public function getSearchKeyword(Request $request)
    {
        try {
            ...
            if ($request->has('idList') && $inputs['idList']) {
                $data = DTGView::scopeSearchByID(explode(',',$inputs['idList']), $isActive, $isDeleted);
            }
            else {
                $data = DTGView::
                scopeSearchByWord(
    ...
    public function AddData(Request $request)
    {
        try {
            ...
            if (!empty($dataID)) {
                ...
                $data = new DrugTsGroup;
            }
            $rules = [
                'name' => 'required|string|max:255|unique:drug_ts_groups,name,' . $dataID,
            ];
    ...
    {
        try {
            Excel::import(new DTGImport, $request->file);
    ...
    public function ChangeStatus(Request $request)
    {
        try {
            ...
            $data = DrugTsGroup::scopeDeleteIt($dataID, $inputs["is_active"] == true ? 1 : 0, $inputs["deleted"] == true ? 1 : 0);
            switch ($data) {
    ...
    public function RemoveIdList(Request $request)
    {
        try {
            ...
            $data = DrugTsGroup::whereIn('id', $dataID['dataID']);
            $data->delete();
            $message = "Data removed successfully";
            return _sendResponse(201, $message);
        } catch (\Throwable $error) {
    ...
    public function RemoveListStatus(Request $request)
    {
        try {
            ...
            $isActive = $inputs["is_active"] == true ? 1 : 0;
            $isDeleted = $inputs["deleted"] == true ? 1 : 0;
            $data = DrugTsGroup::scopeActiveDeactivateList($inputs['dataID'], $isActive, $isDeleted);
            if ($data) {
