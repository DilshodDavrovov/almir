...
use App\Imports\MFImport;
use App\Models\Manufacturer;
use App\Models\Views\ManufacturerView;
...
class ManufacturerController extends Controller
{
    ...
    public function GetByID(Request $request)
    {
        try {
            ...
            $data = Manufacturer::with('country:id,name')->findOrFail($dataID);
            if (empty($data)) {
                $message = "Что-то пошло не так! Пожалуйста, попробуйте позже!";
                $error = "Data not found";
                return _sendError(404, $message, $error);
    ...
    public function getSearchKeyword(Request $request)
    {
        try {
            ...
            if ($request->has('idList') && $inputs['idList']) {
                $data = ManufacturerView::scopeSearchByID(explode(',',$inputs['idList']), $isActive, $isDeleted);
            }
            else {
                $data = ManufacturerView::
                scopeSearchByWord(
                    Str::upper($keyword),
                    $dtIDList,
                    $distID,
    ...
    public function AddData(Request $request)
    {
        try {
            ...
            if (!empty($dataID)) {
                $data = Manufacturer::findOrFail($dataID);
            } else {
                $data = new Manufacturer;
            }
            $rules = [
                'name' => 'required|string|max:255|unique:manufacturers,name,' . $dataID,
            ];
    ...
    {
        try {
            Excel::import(new MFImport, $request->file);
    ...
    public function ChangeStatus(Request $request)
    {
        try {
            ...
            $data = Manufacturer::scopeDeleteIt($dataID, $inputs["is_active"] == true ? 1 : 0, $inputs["deleted"] == true ? 1 : 0);
            switch ($data) {
    ...
    public function RemoveIdList(Request $request)
    {
        try {
            ...
            $data = Manufacturer::whereIn('id', $dataID['dataID']);
            $data->delete();
            $message = "Data removed successfully";
            return _sendResponse(201, $message);
        } catch (\Throwable $error) {
    ...
    public function RemoveListStatus(Request $request)
    {
        try {
            ...
            $isActive = $inputs["is_active"] == true ? 1 : 0;
            $isDeleted = $inputs["deleted"] == true ? 1 : 0;
            $data = Manufacturer::scopeActiveDeactivateList($inputs['dataID'], $isActive, $isDeleted);
            if ($data) {
