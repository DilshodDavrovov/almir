...
use App\Models\Views\RegionView;
...
class RegionController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        ->where(function ($q) use ($inputs, $request) {
            $q->where([
                ['regions.is_active', $inputs["is_active"] == true ? 1 : 0],
                ['regions.deleted', $inputs["deleted"] == true ? 1 : 0],
            ]);

            if ($request->has('country_id') && $inputs['country_id']) {
                $q->where('regions.country_id', $inputs["country_id"]);
            }

            if ($request->has('dataID') && !empty($inputs['dataID'])) {
                $q->whereIn('regions.id', $inputs['dataID']);
            }
        });
    ...
    public function GetByID(Request $request)
    {
        try {
            ...
            $data = Region::with('country:id,name')->findOrFail($dataID);
            if (empty($data)) {
                $message = "Что-то пошло не так! Пожалуйста, попробуйте позже!";
                $error = "Data not found";
                return _sendError(404, $message, $error);
    ...
    public function getSearchKeyword(Request $request)
    {
        try {
            ...
            $CountryId = $inputs['country_id'] ?? null;
            ...
            if ($request->has('idList') && $inputs['idList']) {
                $data = RegionView::scopeSearchByID(explode(',', $inputs['idList']), $isActive, $isDeleted, $CountryId);
            } else {
                $data = RegionView::scopeSearchByWord(
                    Str::upper($keyword),
                    $dtIDList,
                    $distID,
                    $scID,
    ...
    public function AddData(Request $request)
    {
        try {
            ...
            $data->country_id = Str::upper($inputs['country_id']);
    ...
    public function ChangeStatus(Request $request)
    {
        try {
            ...
            $data = Region::scopeDeleteIt($dataID, $inputs["is_active"] == true ? 1 : 0, $inputs["deleted"] == true ? 1 : 0);
            switch ($data) {
    ...
    public function RemoveIdList(Request $request)
    {
        try {
            ...
            $data = Region::whereIn('id', $dataID['dataID']);
            $data->delete();
            $message = "Data removed successfully";
            return _sendResponse(201, $message);
        } catch (\Throwable $error) {
    ...
    public function RemoveListStatus(Request $request)
    {
        try {
            ...
            $data = Region::scopeActiveDeactivateList($inputs['dataID'], $isActive, $isDeleted);
            if ($data) {
