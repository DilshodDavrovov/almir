...
class DistrictView extends Model
{
    ...
    public static function scopeSearchByWord(
        ...
        if (!empty($distID)) {
            $data = $data->whereIn('id', function ($query) use ($distID) {
                $query->select('drug_reports.district_id')->distinct()->from('drug_reports')
    ...
    public static function scopeSearchByID($id, $isActive, $isDeleted, $RegionId)
    {
        $query = [
            ['is_active', $isActive],
            ['is_deleted', $isDeleted]
